...
_DECOMPOSITION_FIELD_WEIGHTS = {
    "problem": 0.04,
    "solution": 0.08,
    "implementation": 0.08,
    "performance_targets": 0.05,
    "legal_foundation": 0.03,
...
class AlignmentCandidateMatch:
    ...
    def to_review_queue_payload(self) -> dict[str, Any]:
        ...
        return {
            ...
            "graph_relations": [
                {
                    ...
                    "relationship_properties": provenance_properties,
            ...
            "review_context": {"workflow": {"workflow_kind": "agenda_promise_alignment_review", "reviewed_alignment_status": "reviewed_approved", "candidate_generation_method": "deterministic_rules_v3", "score_breakdown": self.score_breakdown, "shared_tokens": self.shared_tokens, "category_match": self.category_match, "responsible_entity_match": self.responsible_entity_match, "timeline_match": self.timeline_match}, "agenda_item": self.agenda_snapshot, "political_promise": self.political_promise_snapshot},
...
def _extract_payload_text(payload: Any) -> str:
    ...
    if isinstance(payload, str):
        ...
        try:
            ...
            data = stripped
    fragments = _flatten_text_fragments(data)
...
def _score_decomposition_overlap(agenda: dict[str, Any], promise_tokens: set[str]) -> tuple[float, dict[str, float], list[str]]:
    breakdown: dict[str, float] = {}
    shared_signal_tokens: list[str] = []
    total = 0.0
    for field_name, max_weight in _DECOMPOSITION_FIELD_WEIGHTS.items():
...
def score_alignment_candidate(agenda: dict[str, Any], promise: dict[str, Any], *, approval_threshold: float = _DEFAULT_APPROVAL_THRESHOLD, review_threshold: float = _DEFAULT_REVIEW_THRESHOLD) -> AlignmentCandidateMatch | None:
    ...
    agenda_primary_tokens = set(tokenize_text(agenda.get("title"), agenda.get("description"), agenda.get("summary")))
    promise_tokens = set(tokenize_text(promise.get("title"), promise.get("summary"), promise.get("description")))
    shared_tokens = sorted(agenda_primary_tokens & promise_tokens)
    ...
    token_overlap_score = _token_jaccard(agenda_primary_tokens, promise_tokens)
    signal_overlap_score = _token_jaccard(_signal_tokens(agenda_primary_tokens), _signal_tokens(promise_tokens))

    decomposition_score, decomposition_breakdown, decomposition_shared_tokens = _score_decomposition_overlap(agenda, _signal_tokens(promise_tokens))
    total_signal_shared_count = len(set(signal_shared_tokens) | set(decomposition_shared_tokens))
    ...
    if category_match:
        has_enough_signal = total_signal_shared_count >= _MIN_SIGNAL_SHARED_TOKENS or signal_overlap_score >= 0.12 or decomposition_score >= 0.05 or timeline_match or responsible_entity_match
        if not has_enough_signal:
            return None
    else:
        if shared_count < _MIN_SHARED_TOKENS and decomposition_score < 0.07:
            return None
        if total_signal_shared_count < _MIN_SIGNAL_SHARED_TOKENS and decomposition_score < 0.05:
            return None
        if signal_overlap_score < 0.14 and decomposition_score < 0.05 and not timeline_match and not responsible_entity_match:
            return None
    ...
    if decomposition_shared_tokens:
        notes_parts.append("agenda decomposition overlap: " + ", ".join(decomposition_shared_tokens[:8]))
    if timeline_match:
    ...
    return AlignmentCandidateMatch(
        ...
        score_breakdown={
            ...
            "decomposition_overlap": decomposition_score,
        ...
        shared_tokens=sorted(dict.fromkeys((signal_shared_tokens or shared_tokens) + decomposition_shared_tokens)),
        ...
        agenda_snapshot={
            ...
            "problem": _extract_payload_text(agenda.get("problem")),
            "solution": _extract_payload_text(agenda.get("solution")),
            "implementation": _extract_payload_text(agenda.get("implementation")),
            "performance_targets": _extract_payload_text(agenda.get("performance_targets")),
            "legal_foundation": _extract_payload_text(agenda.get("legal_foundation")),
            "real_world_evidence": _extract_payload_text(agenda.get("real_world_evidence")),
        },
